...
type GoalsScreenTab = 'guided' | 'goals'

const GOALS_VIEW_MODE_STORAGE_KEY = 'automoney:goals:view-mode'
const GOALS_SCREEN_TAB_STORAGE_KEY = 'automoney:goals:screen-tab'
...
const getInitialGoalsScreenTab = (): GoalsScreenTab => {
  if (typeof window === 'undefined') return 'guided'

  const stored = window.localStorage.getItem(GOALS_SCREEN_TAB_STORAGE_KEY)
  return stored === 'goals' ? 'goals' : 'guided'
}
...
export function GoalsView({ onNavigate }: GoalsViewProps) {
  ...
  const [activeTab, setActiveTab] = useState<GoalsScreenTab>(getInitialGoalsScreenTab)
  ...
  useEffect(() => {
    if (typeof window === 'undefined') return
    window.localStorage.setItem(GOALS_SCREEN_TAB_STORAGE_KEY, activeTab)
  }, [activeTab])
      ...
      <div className="space-y-3 pb-2 pt-4">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div className="space-y-3">
            ...
            <div className="app-pill inline-flex items-center gap-1 rounded-2xl p-1">
              <button
                onClick={() => setActiveTab('guided')}
                className={`rounded-xl px-4 py-2 text-sm font-medium transition-all ${activeTab === 'guided'
              ...
              <button
                ...
                className={`rounded-xl px-4 py-2 text-sm font-medium transition-all ${activeTab === 'goals'
                    ? 'bg-[var(--app-accent)] text-[var(--app-accent-foreground)]'
                    : 'text-[var(--app-text-muted)] hover:text-[var(--app-text)]'
                  }`}
              >
          ...
          <div className="flex flex-col gap-3 sm:flex-row">
            ...
            >
              ...
              {activeTab === 'guided' ? 'Meta manual' : 'Nova meta'}
            </button>
          </div>
        </div>
      </div>
                    ...
                    {healthSummary.criteria.map((criterion) => (
                      ...
                      >
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <span
                            className={`shrink-0 rounded-full px-2.5 py-1 text-[11px] font-semibold ${criterion.met
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                <div className="app-pill flex items-center gap-2 rounded-2xl p-1">
                  <button
                    ...
                    className={`rounded-xl px-6 py-2 font-medium transition-all ${viewMode === 'month'
                        ? 'bg-[var(--app-accent)] text-[var(--app-accent-foreground)]'
                        : 'text-[var(--app-text-muted)] hover:text-[var(--app-text)]'
                      }`}
                  >
                  ...
                  <button
                    ...
                    className={`rounded-xl px-6 py-2 font-medium transition-all ${viewMode === 'year'
                        ? 'bg-[var(--app-accent)] text-[var(--app-accent-foreground)]'
                        : 'text-[var(--app-text-muted)] hover:text-[var(--app-text)]'
                      }`}
                  >
